...
-- Run this script directly on MySQL database

USE originmcp;

-- Clean up existing calculator tools
...
-- Verify
SELECT 'Registered Tools:' AS '';
SELECT tool_name, description, service_bean_name, method_name FROM mcp_tool_definition;
SELECT 'Tool Parameters:' AS '';
SELECT td.tool_name, p.parameter_name, p.parameter_type, p.parameter_order
FROM mcp_tool_parameter p
JOIN mcp_tool_definition td ON p.tool_id = td.id
ORDER BY td.tool_name, p.parameter_order;
